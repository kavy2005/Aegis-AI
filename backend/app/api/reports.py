...
from app.schemas.schemas import (
    ReportUploadResponse, ExtractedParameterOut, AnalyzeRequest,
    ReportAnalysisResponse, RiskAssessmentOut, ReportSummaryOut,
)
...
async def upload_report(
    ...
    try:
        result = extract_and_normalize(file_bytes, file.filename)
    except OCRUnavailableError as e:
        raise HTTPException(status_code=503, detail=str(e))
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))

    report = Report(patient_id=patient.id, filename=file.filename, raw_text=result["raw_text"])
    db.add(report)
    db.commit()
    db.refresh(report)
    ...
    return ReportUploadResponse(
        ...
        parameters=[ExtractedParameterOut.model_validate(p) for p in param_rows],
    )
...
def analyze_report(
    ...
    patient = _get_patient(current_user, db)
    report = db.query(Report).filter(Report.id == report_id, Report.patient_id == patient.id).first()
    if not report:
        raise HTTPException(status_code=404, detail="Report not found")

    if payload and payload.corrections:
        by_label = {c.raw_label: c for c in payload.corrections}
        for param in report.parameters:
            if param.raw_label in by_label:
                ...
                param.canonical_parameter = correction.canonical_parameter or param.canonical_parameter
    ...
    if not report.parameters:
        raise HTTPException(
            status_code=422,
            detail="No parameters could be extracted from this report, so there is nothing to "
                   "analyze. This usually means the scan quality was too low for OCR, or the "
                   "report layout wasn't recognized. Try a clearer scan or a text-based PDF.",
        )
    ...
    result = analyze_findings(rows, sex=patient.sex, language=report.language or "en")

    for param, finding in zip(report.parameters, result["findings"]):
    ...
    risk = result["risk"]
    assessment = db.query(RiskAssessment).filter(RiskAssessment.report_id == report.id).first()
    if not assessment:
        assessment = RiskAssessment(report_id=report.id)
        db.add(assessment)
    ...
    return ReportAnalysisResponse(
        ...
        parameters=[ExtractedParameterOut.model_validate(p) for p in report.parameters],
...
def list_reports(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    patient = _get_patient(current_user, db)
    return sorted(patient.reports, key=lambda r: r.uploaded_at, reverse=True)
...
def get_report(report_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return ReportAnalysisResponse(
        ...
        filename=report.filename,
        raw_text_preview=(report.raw_text or "")[:2000],
        risk=RiskAssessmentOut.model_validate(report.assessment),
        parameters=[ExtractedParameterOut.model_validate(p) for p in report.parameters],
